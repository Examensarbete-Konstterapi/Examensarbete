...
export async function getBookingById(req: Request, res: Response) {
  try {
    ...
    const booking = await BookingModel.findById(id).populate("sessionId").populate("userId", "first_name last_name email");
...
export async function createBooking(req: AuthRequest, res: Response) {
  try {
		...
		if (existingBooking) {
			return res.status(400).json({ error: "User have already booked this session" });
		}
